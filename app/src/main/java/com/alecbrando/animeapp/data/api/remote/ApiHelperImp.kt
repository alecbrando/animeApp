...
class ApiHelperImp @Inject constructor(private val apiService: ApiService) : ApiHelper {
    override suspend fun getTopAnime(sort_type: String) = apiService.getTopAnime(sort_type)
    override suspend fun getAnimeById(mal_id: Int): Response<AnimeDetail> = apiService.getAnimeById(mal_id)
    override suspend fun getQueriedAnime(query: String): Response<QueriedAnime> = apiService.getQueriedAnime(query)
}